...
- Use the GAPS list below. If a listed gap makes a condition's expected behaviour unknown, that condition MUST be "unresolved" and its gap_reference should state the missing information. Example: if the tag must appear for a second offer but the exact tag copy is unspecified, a condition checking that the tag APPEARS may be "resolved", but a condition checking the EXACT tag text must be "unresolved".
