...
| [058](058-clarification-agent.md) | ClarificationAgent (Phase 41B): converts existing GapReport into 3-7 answerable questions (severity->priority deterministic, LLM shapes answer-type), applies answers by augmenting requirements (not regenerating), records assumptions, reuses compute_readiness; text-only, reuses run_structured_stage, no protected-code changes | Accepted |
...
| [064](064-capability-driven-provider-eligibility.md) | Capability-driven provider eligibility (Phase C): remove `exclude_image_providers`; a provider is eligible for a stage iff it satisfies the stage's real capabilities, so multimodal providers participate in both image and downstream text stages; chain order/priorities/failover unchanged; no NVIDIA→Gemini special case | Accepted |
